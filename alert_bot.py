...
# Reads PORT set by Render (e.g., 10000). Uses 10000 as a safe default.
# The previous ValueError (invalid literal for int()) is prevented by ensuring
# PORT is NOT set to an empty string in the Render Config Vars.
...
health_server_runner = None # Holds the aiohttp server runner for cleanup
...
async def monitor_prices(application):
    ...
    while True:
        try:
            ...
            # Timeout handles inactivity (sends WARNING:__main__:WebSocket timeout. Reconnecting...)
...
async def setalert_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /setalert command (Includes fix for invalid syntax)"""
    try:
...
async def price_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Simple feedback
        await update.message.reply_text(f"Requesting current price for {symbol}. Wait a moment...")
...
async def shutdown_async(application: Application):
    ...
    # 2. Close Deriv WebSocket connection (Explicitly awaiting close)
    if deriv_ws and not deriv_ws.closed:
    ...
    if health_server_runner:
        ...
        try:
            ...
            logger.warning(f"Error during health server cleanup: {e}")
